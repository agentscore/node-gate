...
describe('TTLCache', () => {
  ...
  it('evicts expired entries when maxSize is reached', () => {
    const cache = new TTLCache<string>(1000, 3);
    cache.set('a', '1');
    cache.set('b', '2');
    cache.set('c', '3');
    // Expire all entries
    vi.advanceTimersByTime(1001);
    // This should sweep expired entries, then insert
    cache.set('d', '4');
  ...
  it('evicts oldest entries when maxSize is reached and nothing is expired', () => {
    const cache = new TTLCache<string>(60000, 3);
    cache.set('a', '1');
    cache.set('b', '2');
    cache.set('c', '3');
    // Nothing expired, so oldest gets evicted
    cache.set('d', '4');
    expect(cache.get('a')).toBeUndefined();
    expect(cache.get('b')).toBe('2');
